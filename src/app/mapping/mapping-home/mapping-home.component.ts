...
export class MappingHomeComponent implements OnInit {
  ...
  ngOnInit() {
    this.srcPdt = {
      ...
      sCountry: "India",
      sPrice: 20,
      sDescription: "good product",
      sManufacturer: "Agility",
      sName: "Product1",
    };
    this.dstPdt = {
      dId: 1,
      dCountry: "India",
      dPrice: 20,
      dDescription: "good product",
      dManufacturer: "Agility",
      dName: "Product1",
    };
    this.addNode();
    this.connections = this.appService.mappings;
  }
  ...
  addNode() {
    for (var property in this.srcPdt) {
      if (this.srcPdt.hasOwnProperty(property)) {
        this.srcNodes.push({ id: property, name: property, type: "Source" });
      }
    }
